...
vim.api.nvim_create_autocmd("LspAttach", {
  callback = function(ev)
    ...
    -- keymap("n", "<leader>r", lsp.buf.references, opt("Show References"))
    keymap("n", "cd", vim.diagnostic.open_float, opt("Open diagnostic in float"))
    ...
    keymap("n", "<Leader>e", function() vim.diagnostic.jump({ count = 1, float = true }) end, opt("Next Diagnostic"))
    keymap("n", "<Leader>E", function() vim.diagnostic.jump({ count =-1, float = true }) end, opt("Prev Diagnostic"))
    -- stylua: ignore end
  end,
})
-- }}}
...
vim.api.nvim_create_autocmd("FileType", {
  pattern = "python",
  callback = function()
    -- local ok, venv = pcall(require, "extras.venv")
    -- if ok then
    --   venv.setup()
    -- end
